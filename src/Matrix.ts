 ...
 */
 ...
 export class TSMT$Matrix 
 {
  ...
  public sumColumns(): Array<number>
  {
    ...
    let sum: Array<number> = this._matrix[0].slice();
   ...
   */
   public subtractFrom(matrix: TSMT$Matrix): TSMT$Matrix
   {
     ...
     var theMatrix:TSMT$Matrix = this.clone();
   ...
   */
   public timesScalar(s: number): void 
   {
     ...
     var m: number = this._matrix.length;
     let i: number;
     let j: number;
     let row: Array<number>;
   ...
   */
   ...
   {
     var temp:TSMT$Matrix = this.clone();
   ...
   */
   ...
   protected __LU(): number
   {
     ...
     let j: number;
     let k: number;
     let imax: number;  // think result from good, old-fashioned Linpack IDAMAX
